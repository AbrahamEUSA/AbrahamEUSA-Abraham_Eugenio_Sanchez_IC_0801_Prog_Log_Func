...
List<Integer> numbers =List.of(12,9,13,4,6,2,4,12,15);
    ...
    private static void ContainingSpring(List<String> strings){
        strings.stream().filter(FP_Functional_Exercises::selectWords)
        .forEach(FP_Functional_Exercises::Print);
        System.out.println("");
    }
    private static void WordsFourFunctional(List<String> strings){
        strings.stream().filter(FP_Functional_Exercises::WordsFour).
        forEach(FP_Functional_Exercises::Print);
        System.out.println("");
    }
    ...
    private static void NumberCharacter(List<String> strings){
       strings.stream().map(FP_Functional_Exercises::NumberChar)
       .forEach(FP_Functional_Exercises::Print);
    }
